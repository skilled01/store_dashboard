<template>
  <custom-table :headers="headers" :list="store.taxes" has-actions>
    {{ translate('Taxes') }}
    ...
    <template #actions>
      ...
      <add-tax-modal :data="data">{{translate('Tax')}}</add-tax-modal>
    </template>
...
import AddTaxModal from "@/components/custom/Tax/add-tax-modal.vue";

const { t, te } = useI18n();

let headers = ['id', 'name', 'type', 'value', 'status', 'date', 'Actions'];
